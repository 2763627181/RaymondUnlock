...
- Listado al por mayor: `wholesale_products` y `wholesale_contacts` se leen públicamente **solo los activos** y los escribe el admin; `wholesale_orders` y `wholesale_order_items` son solo del admin y los inserta el servidor con `service_role` (código con prefijo `PM` en `next_request_code`). Los renglones guardan nombre, categoría, condición y precio tal como se vendieron, así que un producto puede borrarse después (`product_id` queda nulo).
